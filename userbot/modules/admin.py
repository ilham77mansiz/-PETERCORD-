...
from . import CmdHelp, register, edit_or_reply
from userbot.PetercordUserbot.Petercord import command
# =================== CONSTANT ===================
...
@register(pattern="setgetpic ?(.*)", outgoing=True)
@register(pattern="setgetpic ?(.*)", ilhammansiz=True)
async def set_group_photo(gpic):
    if not gpic.is_group:
        await edit_or_reply(gpic, "`Petercord Mohon Lakukan Perintah Ini Di Grup.`")
        return
    ...
    if not admin and not creator:
        return await edit_or_reply(gpic, NO_ADMIN)

    if replymsg and replymsg.media:
        await edit_or_reply(gpic, "`Mengubah Profil Grup`")
...
@register(pattern="promote ?(.*)", outgoing=True)
@register(pattern="promote ?(.*)", ilhammansiz=True)
async def promote(promt):
    ...
    if not admin and not creator:
        return await edit_or_reply(promt, NO_ADMIN)
    ...
    await edit_or_reply(promt, "`Promosikan Pengguna Sebagai Admin... Mohon Menunggu`")
    ...
    try:
        ...
        await edit_or_reply(promt, "`Berhasil Mempromosikan Pengguna Ini Sebagai Admin!`")
        await sleep(5)
        await promt.delete()
    ...
    # we don't have Promote permission
        ...
        return await edit_or_reply(promt, NO_PERM)
...
@register(pattern="ban ?(.*)", outgoing=True)
@register(pattern="ban ?(.*)", ilhammansiz=True)
async def ban(bon):
    ...
    await edit_or_reply(bon, "`Melakukan Banned!`")
    ...
    try:
        ...
        return await edit_or_reply(bon,
...
@register(pattern="unban ?(.*)", outgoing=True)
@register(pattern="unban ?(.*)", ilhammansiz=True)
async def nothanos(unbon):
    ...
    # Well
    if not admin and not creator:
        return await edit_or_reply(unbon, NO_ADMIN)

    # If everything goes well...
    await edit_or_reply(unbon, "`Sedang Melakukan Unban...`")
    ...
    try:
        ...
        await edit_or_reply(unbon, "```Unban Berhasil Dilakukan!```")
        await sleep(3)
        await unbon.delete()
        ...
        await edit_or_reply(unbon, "`Sepertinya Terjadi Kesalahan!`")
...
CmdHelp("admin").add_command(
    'setgpic',
    '<reply to image>',
    'Mengubah gambar tampilan grup').add_command(
        'promote',
        '<username/reply> <peringkat kustom (opsional)>',
        'Memberikan hak admin kepada seseorang dalam obrolan.').add_command(
            'demote',
            '<username/reply>',
            'Mencabut izin admin orang dalam obrolan.').add_command(
                'ban',
                '<username/reply> <reason (opsional)>',
                'Bans orang tersebut dari chat Anda.').add_command(
                    'unban',
                    '<username/reply>',
                    'Menghapus larangan dari orang dalam obrolan.').add_command(
                        'mute',
                        '<username/reply> <reason (opsional)>',
                        'Bungkam orang dalam obrolan, juga berfungsi di admin.').add_command(
                            'unmute',
                            '<username/reply>',
                            'Menghapus orang tersebut dari daftar yang dibisukan.').add_command(
                                'pin',
                                '<reply> atau .pin loud',
                                'Pin pesan yang dibalas di Grup').add_command(
                                    'kick',
                                    '<username/reply>',
                                    'tendang orang itu dari obrolan Anda').add_command(
                                        'zombies',
                                        None,
                                        'Untuk mencari akun terhapus dalam grup.').add_command(
                                            'zombies clean',
                                            None,
                                            'menghapus akun terhapus dalam grup').add_command(
                                                'bots',
                                                None,
                                                'melihat daftar bot.').add_command(
                                                    'all',
                                                    None,
    'tag all.').add()
